...
// 2026-09-18: Byron reviewed several brand-matched mockups against the
// live site's actual palette and picked a plainer final version — this
// supersedes the 2026-09-13 "confident hierarchy" pass below. Changes:
// (1) colors pulled directly from the live site's computed styles rather
// than the earlier hand-picked blues/greens — brand blue #1476a8, navy
// #152033, salary green #067647 (matching the /careers job-card salary
// exactly), so the email card now matches the site instead of running a
// separate palette; (2) eyebrow shortened to "AGILE OPPORTUNITY" (dropped
// "CAREER"); (3) both links are now the SAME color and size — no more
// "quieter second link" treatment — with "View Position" simply bolder
// (700 vs 400) as the one primary action, and "See similar positions"
// shortened to "Similar Positions" (dropped "See", title case to match
// "View Position"); (4) the arrow on "View Position" was dropped along
// with the old two-tone-blue design it was styled for; (5) letter-spacing
// on the eyebrow was dropped — it was only ever set on the outer <div>,
// which Outlook's paste sanitizer discards anyway, so it did nothing in
// the one client this card gets the most real-world scrutiny in; (6) the
// smallest sizes (eyebrow, position ID) were floored at 12px instead of
// 10–11px for legibility once opened on a phone screen, since that's
// where most recipients actually read these.
//
// The original 2026-09-13 rationale for the triple-redundant styling
// below still applies in full — see that history in git blame if needed.
...
// color. Every visual property below is therefore writen THREE times:
...
// Outlook Wed, that strip the div and keep only inline/character-level
// formatting).
const colored = (text, { size, weight = 400, color, margin = "0 0 4px" }) => {
...
return `<div style="font-family:Arial,Helvetica,sans-serif;font-size:${size};font-weight:${weight};color:${color};line-height:1.45;margin:${margin}"><span style="${sizeWeightStyle}"><span style="color:${color}"><font color="${color}">${text}</font></pan></pan></div>`;
};

const eyebrow = colored("AGILE OPPORTUNITY", { size: "12px", weight: 700, color: "#1476a8", margin: "0 0 5px" });
const titleLine = colored(title, { size: "16px", weight: 700, color: "#152033", margin: "0 0 5px" });
const detail = detailLine ? colored(detailLine, { size: "13px", weight: 400, color: "#5d6b78", margin: "0 0 5px" }) : "";
const salaryLine = salary ? colored(salary, { size: "18px", weight: 800, color: "#067647", margin: "0 0 5px" }) : "";
const idLine = id ? colored(`Position ID ${escapeHtml(id)}`, { size: "12px", weight: 400, color: "#7c8894", margin: "0 0 12px" }) : "";

// Both links share the same color and size now — "View Position" is the
// one primary action, set apart by weight alone (700 vs 400), not by a
// second, quieter color the way the 2026-09-13 version did it.
const linkColor = "#1476a8";
const viewSizeWeight = "font-size:14px;font-weight:700;";
const link = `<div style="margin:0 0 4px"><a href="${url}" style="font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:700;color:${linkColor};text-decoration:underline"><span style="${viewSizeWeight}"><span style="color:${linkColor}"><font color="${linkColor}">View Position</font></span></span></a></div>`;

const similarSizeWeight = "font-size:14px;font-weight:400;";
const similarLink = `<div style="margin:0"><a href="${shortSimilarUrl(job)}" style="font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:400;color:${linkColor};text-decoration:underline"><span style="${similarSizeWeight}"><span style="color:${linkColor}"><font color="${linkColor}">Similar Positions</font></span></span></a></div>`;
