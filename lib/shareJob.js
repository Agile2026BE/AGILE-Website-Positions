...
export function positionShareHtml(job) {
  ...
  // rendering.
  //
  // 2026-09-13: even at the "settled" 15px/700, Byron found the title
  // still read as oversized once actually pasted into a real Outlook
  // compose window (next to Outlook's own 12pt/16px default body text,
  // a bold 15px line stood out as a shouty headline rather than sitting
  // in line with the rest of the card). Brought down to 13px/700 — now
  // equal to (not larger than) the salary line and the link lines, so
  // nothing on the card outsizes anything else; bold weight alone still
  // marks it as the title without inflating the point size.
  //
  // 2026-09-13 (later): Position ID line raised from 11px to 12px to
  // match the detail line (location · workplace, e.g. "NJ, NY, and PA ·
  // Hybrid") — Byron wanted the two secondary/metadata lines sized the
  // same instead of Position ID reading smaller than the line above it.
  ...
  const eyebrow = colored("AGILE CAREER OPPORTUNITY", { size: "10px", weight: 700, color: "#1476a8", letterSpacing: ".4px", margin: "0 0 4px" });
  const titleLine = colored(title, { size: "13px", weight: 700, color: "#173958" });
  const detail = detailLine ? colored(detailLine, { size: "12px", weight: 400, color: "#5a7185" }) : "";
  const salaryLine = salary ? colored(salary, { size: "13px", weight: 700, color: "#0a8a4a" }) : "";
  const idLine = id ? colored(`Position ID ${escapeHtml(id)}`, { size: "12px", weight: 400, color: "#6b7684", margin: "0 0 6px" }) : "";

  const linkSizeWeight = "font-size:13px;font-weight:700;";
  const linkColor = "#1476a8";
  const link = `<div style="margin:0 0 3px"><a href="${url}" style="font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;color:${linkColor};text-decoration:underline"><span style="${linkSizeWeight}"><span style="color:${linkColor}"><font color="${linkColor}">View Position</font></span></span></a></div>`;

  const similarSizeWeight = "font-size:13px;font-weight:700;";
  const similarColor = "#1476a8";
  const similarLink = `<div style="margin:0"><a href="${shortSimilarUrl(job)}" style="font-family:Arial,Helvetica,sans-serif;font-size:13px;font-weight:700;color:${similarColor};text-decoration:underline"><span style="${similarSizeWeight}"><span style="color:${similarColor}"><font color="${similarColor}">Similar Positions</font></span></span></a></div>`;
