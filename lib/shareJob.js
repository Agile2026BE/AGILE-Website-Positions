...
// Converts A-Z/a-z/0-9 to their Mathematical Sans-Serif Bold Unicode
// equivalents (U+1D5D4 block). This renders as genuinely bold text in any
// plain-text context — SMS, iMessage, LinkedIn messaging, Juicebox — with
// no HTML, no color, and no size change, so it emphasizes the title
// without making it look like a tappable link or blowing up its size the
// way the earlier ALL CAPS treatment did. Punctuation/spaces pass through
// unchanged since the bold block has no punctuation glyphs.
...
export function positionShareText(job) {
  ...
  // styling of any kind. ALL CAPS was tried as a plain-text stand-in for
  // emphasis but read as too loud/shouty next to normal message text.
  // Unicode bold (see toUnicodeBold) replaces it — genuinely bold-looking
  // without shouting, without looking like a link, and without changing
  // size.
  const title = toUnicodeBold(job?.title ?? "AGILE Career Opportunity");
...
export function positionShareHtml(job) {
  ...
  const eyebrow = colored("AGILE CAREER OPPORTUNITY", { size: "9px", weight: 700, color: "#1476a8", letterSpacing: ".4px", margin: "0 0 4px" });
  const titleLine = colored(title, { size: "13px", weight: 700, color: "#173958" });
  const detail = detailLine ? colored(detailLine, { size: "11px", weight: 400, color: "#5a7185" }) : "";
  const salaryLine = salary ? colored(salary, { size: "12px", weight: 700, color: "#0a8a4a" }) : "";
  const idLine = id ? colored(`Position ID ${escapeHtml(id)}`, { size: "10px", weight: 400, color: "#6b7684", margin: "0 0 6px" }) : "";

  const linkSizeWeight = "font-size:12px;font-weight:700;";
  const linkColor = "#1476a8";
  const link = `<div style="margin:0 0 3px"><a href="${url}" style="font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:700;color:${linkColor};text-decoration:underline"><span style="${linkSizeWeight}"><span style="color:${linkColor}"><font color="${linkColor}">View Position</font></span></span></a></div>`;

  const similarSizeWeight = "font-size:11px;font-weight:600;";
  const similarColor = "#1476a8";
  const similarLink = `<div style="margin:0"><a href="${url}#similar-positions" style="font-family:Arial,Helvetica,sans-serif;font-size:11px;font-weight:600;color:${similarColor};text-decoration:underline"><span style="${similarSizeWeight}"><span style="color:${similarColor}"><font color="${similarColor}">Similar Positions</font></span></span></a></div>`;
