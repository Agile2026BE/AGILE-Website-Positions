...
// upper bound, so by nature they qualify a candidate for that band AND every
// band above it, all the way to the top — a "10+ years" posting is exactly
// the kind of requirement a 20+ year candidate also satisfies. "+" phrasing
// specifically ("10+ years") is read as strictly OVER the stated number per
// Byron's real-world hiring convention (10+ means 11 and up, not 10 and up),
// so its effective threshold is min + 1; "Minimum N years" phrasing is read
// as inclusive at N (unconfirmed assumption — flag if Byron wants "Minimum"
// treated the same as "+"). Either way, matching is open-ended upward: no
// pinning to a single band the way closed ranges are.
//
...
const experienceMatches = (job, selectedBand) => {
  ...
  if (max == null) {
    const hasPlus = /\+/.test(String(job.experience));
    const effectiveMin = hasPlus ? min + 1 : min;
    return effectiveMin <= band.max;
  }
