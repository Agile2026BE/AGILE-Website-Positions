...
// A candidate selects the specific experience band they want to browse, and
// only postings whose own required-experience range overlaps that single
// band are shown — e.g. selecting "11–15 years" shows postings requiring
// 11–15 years, not postings requiring 1–2 or 3–5 years, and not every
// posting above 6–10 either. Open-ended requirements ("10+ years", "Minimum
// 10 years") have no stated upper bound in the source data, so for band
// matching they're pinned to the single band containing their stated
// minimum (a "10+ years" posting matches the 6–10 band, not 11–15 or
// 26–35) — this keeps every posting associated with exactly one band
// instead of bleeding across every band above its minimum. A posting with
// no stated/parseable requirement has no experience barrier, so it always
// shows, regardless of which band is selected.
const experienceMatches = (job, selectedBand) => {
  ...
  const jobMax = max == null ? min : max;
  return min <= band.max && jobMax >= band.min;
};
