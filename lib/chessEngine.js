...
//
// 2026-09-18 update: bestMove() used to break ties on raw material score
// alone, and on AGILE's very first reply every non-capturing move ties at 0
// — so it always grabbed the same first-enumerated knight move. Fixed with
// (1) a small opening book for AGILE's first reply, randomized among a few
// intermediate-level main-line responses to each common first move, and
// (2) a light positional bonus (center control, knight development) plus
// randomized selection among near-equal top moves, so play stays varied and
// more purposeful after the book runs out too.
...
// --- Positional heuristics (opening/middlegame flavor) -----------------
// Small, deliberately conservative bonuses layered on top of material so
// that quiet, non-capturing moves aren't all scored as flat ties — that
// flat-tie behavior was the root cause of the AI always repeating the same
// first move. None of this outweighs material; it just breaks ties toward
// sensible, human-recognizable development.

const CENTRAL = new Set(["d4", "d5", "e4", "e5"]);
const NEAR_CENTRAL = new Set(["c3", "c4", "c5", "c6", "d3", "d6", "e3", "e6", "f3", "f4", "f5", "f6"]);
...
function squareBonus(r, c, type) {
  const s = algebraic(r, c);
  let bonus = 0;
  if (type === "P" || type === "N" || type === "B") {
    if (CENTRAL.has(s)) bonus += 6;
    else if (NEAR_CENTRAL.has(s)) bonus += 3;
  }
  if (type === "N" && (s[0] === "a" || s[0] === "h")) bonus -= 8;
  if (type === "B" && (s[0] === "a" || s[0] === "h") && (s[1] === "1" || s[1] === "8")) bonus -= 4;
  return bonus;
}
...
function evaluate(state) {
  ...
  for (let r = 0; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      const p = state.board[r][c];
      if (!p) continue;
      const v = VALUES[p.type] + squareBonus(r, c, p.type);
...
// --- Opening book (AGILE's first reply only) ----------------------------
// Keyed by White's first move (piece + from-square + to-square). Each entry
// is a short list of solid, intermediate-level main-line replies; one is
// picked at random so the game doesn't open the same way every time. Any
// first move not covered here (rare tries like 1.a4) just falls through to
// the positional search below, which still varies its pick.

function pm(fromFile, fromRank, toFile, toRank, opts = {}) {
  return { from: coord(fromFile, fromRank), to: coord(toFile, toRank), ...opts };
}

const OPENING_BOOK = {
  Pe2e4: [pm("e", 7, "e", 5, { double: true }), pm("c", 7, "c", 5, { double: true }), pm("e", 7, "e", 6), pm("c", 7, "c", 6)],
  Pd2d4: [pm("d", 7, "d", 5, { double: true }), pm("g", 8, "f", 6), pm("e", 7, "e", 6)],
  Pc2c4: [pm("e", 7, "e", 5, { double: true }), pm("g", 8, "f", 6), pm("c", 7, "c", 5, { double: true })],
  Ng1f3: [pm("d", 7, "d", 5, { double: true }), pm("g", 8, "f", 6), pm("c", 7, "c", 5, { double: true })],
  Nb1c3: [pm("d", 7, "d", 5, { double: true }), pm("g", 8, "f", 6)],
  Pg2g3: [pm("d", 7, "d", 5, { double: true }), pm("e", 7, "e", 5, { double: true })],
  Pb2b3: [pm("e", 7, "e", 5, { double: true }), pm("d", 7, "d", 5, { double: true })],
  Pe2e3: [pm("d", 7, "d", 5, { double: true }), pm("g", 8, "f", 6)],
  Pd2d3: [pm("e", 7, "e", 5, { double: true }), pm("g", 8, "f", 6)],
};
...
// Detects "it's Black to move, and White has played exactly one move so
// far" by checking Black's ranks are untouched and exactly one White piece
// has shifted from its starting square. Returns that move's piece type and
// squares, or null if this isn't AGILE's very first reply.
function detectFirstWhiteMove(state) {
  if (state.turn !== BLACK) return null;
  const init = initialBoard();
  if (!ranksEqual(state.board[0], init[0])) return null;
  if (!ranksEqual(state.board[1], init[1])) return null;
  ...
  for (let r = 2; r < 8; r++) {
    for (let c = 0; c < 8; c++) {
      ...
      const ini = init[r][c];
      const same = (!cur && !ini) || (cur && ini && cur.type === ini.type && cur.color === ini.color);
      if (!same) diffs.push([r, c]);
    }
  }
  ...
  for (const [r, c] of diffs) {
    if (!state.board[r][c]) fromSq = [r, c];
    else toSq = [r, c];
  }
  if (!fromSq || !toSq) return null;
  const piece = state.board[toSq[0]][toSq[1]];
  if (!piece || piece.color !== WHITE) return null;
  return { from: fromSq, to: toSq, type: piece.type };
}
...
export function bestMove(state, depth) {
  ...
  if (state.turn === BLACK) {
    const opening = detectFirstWhiteMove(state);
    if (opening) {
      const key = `${opening.type}${algebraic(opening.from[0], opening.from[1])}${algebraic(opening.to[0], opening.to[1])}`;
      const options = OPENING_BOOK[key];
      if (options && options.length) {
        const valid = options
          .map((o) => moves.find((m) => m.from[0] === o.from[0] && m.from[1] === o.from[1] && m.to[0] === o.to[0] && m.to[1] === o.to[1]))
          .filter(Boolean);
        if (valid.length) {
  ...
  for (const m of moves) {
    ...
    scored.push({ m, score });
    if (maximizing ? score > bestScore : score < bestScore) bestScore = score;
  }

  // Keep every move within a small margin of the best score in play, and
  // pick among them at random, so equally-reasonable moves don't always
  // resolve to the same one every game.
  const MARGIN = 25;
  const top = scored.filter((s) => Math.abs(s.score - bestScore) <= MARGIN);
