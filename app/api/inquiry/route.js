...
export async function POST(request) {
  try {
    ...
    if (!apiKey) return NextResponse.json({ error: "Inquiry delivery is not configured yet. Please call 407-868-7254 or email careers@agileconsultingsolutions.com." }, { status: 503 });
    ...
    const payload = {
      ...
      text: [
        ...
        `Position ID: ${positionId || "Not selected"}`,
    ...
    if (!response.ok) {
      ...
      console.error("Inquiry email failed", response.status, detail);
